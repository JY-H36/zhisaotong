...
def set_current_user_id(user_id: str | int) -> None:
    """设置当前线程的用户 ID（线程安全）"""
    _user_context.user_id = str(user_id)
...
def _gaode_get(path: str, params: dict) -> dict:
    ...
    if not gaode_key:
        raise ValueError("agent.yaml 中未配置 gaodekey")

    base_url = agent_config.get("gaode_base_url", "https://restapi.amap.com")
    timeout = float(agent_config.get("gaode_timeout", 10))
    ...
    try:
        ...
        raise RuntimeError(f"高德 HTTP 错误: {e.code}") from e
...
def _resolve_city_to_adcode(city: str) -> tuple[str, str]:
    ...
    if not adcode:
        raise RuntimeError("城市解析成功但未返回 adcode")
...
@tool(description="获取指定城市的天气，返回天气描述字符串")
...
@tool(description="获取用户所在城市的名称，通过 IP 定位返回城市字符串")
...
@tool(description="获取当前登录用户的ID，以纯字符串形式返回。当需要查询用户自己的数据或生成报告时，优先调用此工具获取用户ID。")
def get_user_id() -> str:
    uid = get_current_user_id()
    if uid:
        return uid
    return random.choice(user_ids)  # fallback: 未登录时随机
...
@tool(description="从外部系统中获取用户在指定月份的使用记录。如未提供user_id则自动使用当前登录用户。返回纯字符串，未检索到返回空字符串")
def fetch_external_data(month: str, user_id: str = "") -> str:
    generate_external_data()

    # 如果未提供 user_id，自动使用当前登录用户 ID
    if not user_id:
        uid = get_current_user_id()
        if uid:
            user_id = uid
    ...
    if not user_id:
        logger.warning("[fetch_external_data]未提供用户ID且无法获取当前用户")
        return ""
...
if __name__ == '__main__':
    # @tool 装饰器返回的是 StructuredTool 对象，需用 .invoke() 调用，传入字典参数
    print("=== 测试 get_user_location ===")
